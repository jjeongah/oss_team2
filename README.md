...
1. 새 습관 추가하기 버튼을 눌러 습관을 추가하세요.
   ...
   SPKU(습꾸)는 운동, 공부, 취미, 감정관리 카테고리마다 설정할 습관을 추천해줍니다.
...
-   desktop app으로 사용할 경우, 미입력후 확인하기 버튼을 눌러 팝업창 뜨고 난 이후에 모든 페이지에서 입력이 안되는 버그가 발생합니다.  
    웹에서 실행할 경우에 발생하지 않는 버그라 이후 해결책을 찾는 중입니다.
