    ...
    if (viewMonth === current_month && viewYear === current_year) {
      for (let date of document.querySelectorAll(".this")) {
        if (
          ...
          +date.innerText < current_date + span_choice[k]
        ) {
...
window.addEventListener("load", () => {
  ...
  if (num_called > 0) {
    for (k = 1; k <= num_called; k++) {
      ...
      //contentChild.className = "hlist";
      //contentChild.onclick = select;
      //let habitStr = document.createElement("div");
      //habitStr.innerHTML = "<i>" + stored_arr[k] + "</i>";
      //let habitDay = document.createElement("div");
      //contentChild.appendChild(habitStr);
      //document.getElementsByClassName("contents").appendChild(contentChild);
    }
    //document.getElementById('li1').innerHTML = stored_arr[num_called];
  }
});
